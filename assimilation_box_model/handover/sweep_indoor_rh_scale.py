...
def apply_indoor_temp_and_rh_scale(
    prep: gh.PreparedDay,
    *,
    temp_offset_c: float,
    rh_scale: float,
    ...
    t_in_adj = (prep_series(prep, "t_in_model", "t_in_raw") + float(temp_offset_c)).rename("T_in_model_bulk_C")
    rh_in_adj_pct = (prep_series(prep, "rh_in_model", "rh_in_raw") * float(rh_scale)).clip(lower=0.0, upper=110.0).rename("RH_in_model_bulk_pct")
    omega_in_adj = pd.Series(
        gh.wgc.humidity_ratio(
            rh_in_adj_pct.to_numpy(dtype=float) / 100.0,
            t_in_adj.to_numpy(dtype=float),
            101325.0,
        ),
        index=idx,
...
def summarize_case(date_yyyymmdd: str, temp_offset_c: float, rh_scale: float, res: pd.DataFrame) -> dict[str, float | str]:
...
def heatmap_matrix(summary: pd.DataFrame, value_col: str, temp_offsets: list[float], rh_scales: list[float]) -> np.ndarray:
    piv = summary.pivot(index="temp_offset_c", columns="rh_scale", values=value_col)
    piv = piv.reindex(index=temp_offsets, columns=rh_scales)
...
def plot_date_heatmaps(
    ...
    for ax, (col, title) in zip(axes, panels):
        matrix = heatmap_matrix(summary, col, temp_offsets, rh_scales)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sweep indoor temperature offset and RH scaling to mimic canopy-near sensor bias.")
    ap.add_argument("--dates", default="20260405,20260407,20260411")
    ap.add_argument("--temp-offsets", default="0,1,2,3")
    ap.add_argument("--rh-scales", default="1.00,0.95,0.90,0.85,0.80")
    ap.add_argument("--output-dir", default=str(Path(__file__).resolve().parent / "indoor_rh_scale_sweep"))
    args = ap.parse_args()

    dates = parse_csv_list(args.dates, cast=str)
    temp_offsets = parse_csv_list(args.temp_offsets, cast=float)
    rh_scales = parse_csv_list(args.rh_scales, cast=float)
    ...
    for date in dates:
        ...
        for temp_offset_c in temp_offsets:
            for rh_scale in rh_scales:
                prep_mod = apply_indoor_temp_and_rh_scale(
                    prep_base,
                    temp_offset_c=temp_offset_c,
                    rh_scale=rh_scale,
                )
                res = gh.apply_current_model(prep_mod)
                row = summarize_case(date, temp_offset_c, rh_scale, res)
                all_rows.append(row)
                date_rows.append(row)

        date_summary = pd.DataFrame(date_rows).sort_values(["temp_offset_c", "rh_scale"])
        date_dir = output_dir / date
        date_dir.mkdir(parents=True, exist_ok=True)
        csv_path = date_dir / f"{date}_indoor_rh_scale_summary.csv"
        png_path = date_dir / f"{date}_indoor_rh_scale_heatmaps.png"
        date_summary.to_csv(csv_path, index=False, encoding="utf-8-sig")
        plot_date_heatmaps(date_summary, date, temp_offsets, rh_scales, png_path)
    ...
    all_summary = pd.DataFrame(all_rows).sort_values(["date_yyyymmdd", "temp_offset_c", "rh_scale"])
    all_summary_path = output_dir / "indoor_rh_scale_summary_all.csv"
    best_path = output_dir / "indoor_rh_scale_best_by_date.csv"
